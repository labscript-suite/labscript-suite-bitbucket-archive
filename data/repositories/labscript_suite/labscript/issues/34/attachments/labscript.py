...
def bitfield(arrays,dtype):
    ...
    for i in range(1,len(arrays)):#range(1,n[dtype]):
...
def intersect1dtolerance(arr1, arr2, tolerance=1e-10):
    ...
    #VERY slow!!! due to double for-loop
#    result = []
#    for i in arr1:
#        for j in arr2:
    ...
    def ramp(self, t, duration, initial, final, samplerate, units=None, truncation=1.):
        ...
        if truncation > 0:
            ...
            if initial == final:
                ...
                message = ''.join(['WARNING: AnalogOutput \'%s\' has the same initial and final value at time t=%.10f with duration %.10f. To save samples and clock ticks this instruction is replaced with a constant output. '%(self.name, t, duration)])
                sys.stderr.write(message+'\n')
            else:
...
class DigitalQuantity(Output):
    ...
    def __init__(self, name, parent_device, connection, inverted=False, **kwargs):
        ...
        #self.high_delay, self.low_delay = switch_delay
    ...
    def go_high(self,t):
        ...
        if self.inverted:
            ...
            self.add_instruction(t, 1)
        #self.add_instruction(t-self.high_delay if t >= self.high_delay else 0, 1)
    ...
    def go_low(self,t):
        ...
        if self.inverted:
            ...
            self.add_instruction(t, 0)
        #self.add_instruction(t-self.low_delay if t >= self.low_delay else 0, 0)
        ...
        if self.open_state == 1:
            ...
            for time, value in self.instructions.items():
                if time + self.close_delay > t - self.open_delay and value == 0:
                    ...
                    sys.stderr.write(message+'\n')
                    #raise LabscriptError("The shutter is requestes to open too early when it is still not closed from an earlier instruction")
            #self.go_high(tcalc)
        ...
        if self.open_state == 1:
            ...
            for time, value in self.instructions.items():
                if time + self.open_delay > t - self.close_delay and value == 1:
                    ...
                    sys.stderr.write(message+'\n')                    
                    #raise LabscriptError("The shutter is requestes to close too early when it is still not opened from an earlier instruction")            
            #self.go_low(tcalc)  
            self.add_instruction(tcalc, 0)
        elif self.open_state == 0:
            tcalc = t-self.close_delay if t >= self.close_delay else 0
...
def generate_code():
    ...
    with h5py.File(compiler.hdf5_filename) as hdf5_file:
        try:
            ...
            runviewer_grp = hdf5_file.create_group('runviewer')
            runviewer_grp.create_dataset('output_info', shape=(1,))
        except ValueError:
